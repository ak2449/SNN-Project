...
% Chirp parameters
bw = 300e6;                     % Bandwidth (15 MHz to prevent aliasing??)
...
% Define Clutter Targets
n_clutter_points = 5; % Add clutter points

% Random ranges from 0m to 250m
clutter_range = rand(1, n_clutter_points) * 250; 
% All clutter is stationary (0 m/s)
clutter_vel   = zeros(1, n_clutter_points);
% Random, weak RCS values
clutter_rcs   = rand(1, n_clutter_points) * 0.001; 

% Combine targets and clutter into one list 
all_targets_range = [target_range, clutter_range];
all_targets_vel   = [target_vel, clutter_vel];
all_targets_rcs   = [target_rcs, clutter_rcs];
n_total_targets = n_targets + n_clutter_points; 
...
for i = 1:n_chirps % Loop for each chirp
    ...
    for j = 1:n_total_targets
        
        % Get this target's properties
        r0 = all_targets_range(j); 
        v0 = all_targets_vel(j);
        ...
        att = all_targets_rcs(j) / (current_range^4); 
